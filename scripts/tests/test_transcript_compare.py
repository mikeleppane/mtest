...
def test_header_replacement_reaches_line_one_only() -> None:
    """The narrow rewrite mode: first line, first occurrence, nothing else.

    The wide mode above is what an approved path relocation needs, because the
    old prefix appears wherever a source location is printed. A toolchain
    identity is the opposite: it belongs to the provenance header, and the same
    text in a transcript body is report content. A rewrite that reached it
    would turn a real change in what the runner prints into agreement.
    """
    with tempfile.TemporaryDirectory(prefix="mtest-header-replace-") as raw_tmp:
        tmp = Path(raw_tmp)
        before = tmp / "before"
        after = tmp / "after"
        before.mkdir()
        after.mkdir()
        old = b"mojo 1.0.0b2 (2cf4d08a)"
        new = b"mojo 1.0.0b3 (cafef00d)"

        (before / "case.txt").write_bytes(b"# generated by " + old + b"\nPASS\n")
        (after / "case.txt").write_bytes(b"# generated by " + new + b"\nPASS\n")
        header_only = compare_directories(before, after, header_replacement=(old, new))
        if not header_only.ok or header_only.changed_files != ("case.txt",):
            raise AssertionError(
                f"the header rewrite was refused its own line: {header_only.errors}"
            )

        (before / "case.txt").write_bytes(
            b"# generated by " + old + b"\nbuilt by " + old + b"\n"
        )
        (after / "case.txt").write_bytes(
            b"# generated by " + new + b"\nbuilt by " + new + b"\n"
        )
        body = compare_directories(before, after, header_replacement=(old, new))
        if body.ok:
            raise AssertionError("a body-carried identity was erased as header drift")

        wide = compare_directories(before, after, replacement=(old, new))
        if not wide.ok:
            raise AssertionError(
                f"the whole-file rewrite mode changed behaviour: {wide.errors}"
            )

        (before / "case.txt").write_bytes(b"# " + old + b" " + old + b"\n")
        (after / "case.txt").write_bytes(b"# " + new + b" " + new + b"\n")
        twice = compare_directories(before, after, header_replacement=(old, new))
        if twice.ok:
            raise AssertionError("the header rewrite fired more than once on line one")

        both = compare_directories(
            before, after, replacement=(old, new), header_replacement=(old, new)
        )
        if both.ok or "only one replacement mode may be supplied" not in both.errors:
            raise AssertionError("two replacement modes at once were not refused")
