...
class ComparisonTests(unittest.TestCase):
    ...
    def test_the_identity_is_forgiven_in_the_header_and_nowhere_else(self) -> None:
        """A body that prints the toolchain is report content, not provenance.

        Both files below carry the identity twice: once in the header, where
        the generator stamps it, and once in the body, where `TestSuite` or a
        fixture printing what built it would put it. Only the header stamp is
        drift-free. Forgiven everywhere, the second occurrence would let a real
        change in what the runner prints be rewritten into agreement — an
        actual behavioural difference erased as tolerated header drift.
        """
        header = (
            "# generated by scripts/gen_transcripts.py — mojo {} ({}), "
            "linux-x86_64, normalizer v1, fixture passing.mojo, scenario "
            "default — do not hand-edit\n"
        )
        with tempfile.TemporaryDirectory(prefix="mtest-canary-body-") as raw:
            root = Path(raw)
            pinned = root / "pinned"
            candidate = root / "candidate"
            for side, version, commit in (
                (pinned, "x.y.z", "deadbeef"),
                (candidate, "x.y.w", "cafef00d"),
            ):
                side.mkdir()
                (side / "MANIFEST.txt").write_text(
                    "passing--default.txt\n", encoding="utf-8"
                )
                (side / "passing--default.txt").write_text(
                    header.format(version, commit)
                    + "$ mtest tests/fixtures/protocol/passing.mojo\n"
                    + f"TestSuite built by mojo {version} ({commit})\n"
                    + "1 passed in 0.00s\n",
                    encoding="utf-8",
                )
            result = compare_transcript_dirs(pinned, candidate)
        self.assertEqual(result.classification, PROTOCOL_DRIFT, result.detail)
        self.assertIn("-TestSuite built by mojo x.y.z (deadbeef)", result.detail)
        self.assertIn("+TestSuite built by mojo x.y.w (cafef00d)", result.detail)
