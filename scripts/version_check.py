#!/usr/bin/env python3
"""Verify MTEST_VERSION and the pixi manifest version agree.
...
tooling. Nothing else keeps the two in sync — this script is that gate: parse
both, assert they are byte-identical, and assert the agreed value is the
version this repo is currently shipping. Wired into `pixi run ci` so a future
edit to either file that forgets the other fails loudly instead of shipping a
mislabeled artifact.
"""
...
def main() -> int:
    """Assert MTEST_VERSION and pixi.toml's version agree with each other and
    with the version this repo is currently shipping.
    """
    try:
