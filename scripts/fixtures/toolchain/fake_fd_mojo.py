...
does, but strictly: it never execs the real compiler, serves only `build`, and
refuses anything else loudly.

The fd-clamp scenario lowers the child's `RLIMIT_NOFILE` soft limit to a number
chosen so mtest's own pool arithmetic clamps, far below what LLVM needs to link
a Mojo binary. Handing that ceiling to a real `mojo build` would kill the run in
the compiler rather than the scheduler, so this stand-in produces the build
product itself at a cost of a few descriptors.

For each `build <source> -o <out>` it:

1. reads the source's `def test_*` names, in source order, so the fabricated
   report names the file's real tests;
...
canonical path, the same string `mojo build` bakes into a real binary, so mtest's
parser accepts it and the file lands PASS. It answers `--skip-all` with an
all-SKIP listing, so a collection probe reads as a listing rather than a suite
that ran its bodies.

Trailing spaces in the header and summary lines are load-bearing grammar.
...
header path and after `skipped`) are the toolchain's grammar."""
...
def _output_path(args: list[str]) -> str:
    ...
    Raises:
        ValueError: If `-o` is absent or carries no operand, which this stand-in
...
def _test_names(source: str) -> list[str]:
    ...
    Raises:
        ValueError: If the file declares no test. A zero-test report is a
            different verdict class (NO-TESTS), so fabricating one would change
            the scenario's accounting instead of failing it.
    """
