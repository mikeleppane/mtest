"""The mapping from a run's outcomes to a process exit code.

This module owns the mapping from the multiset of run outcomes to the three
codes that mapping alone decides: 1, 5, and 0. Codes 4 (pre-run usage error),
3 (internal error), and 2 (interrupt) are control-flow codes decided in the
session and in main, not here.
