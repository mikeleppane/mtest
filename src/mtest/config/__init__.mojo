...
mojo-path resolution helper, and the shared shell-quoting helpers
(`shell_quote`, `shell_join`) every later layer's reproduce-line rendering
uses. It imports only from `model` — and does not today, because none of this
data needs model's vocabulary. It is DATA plus pure helpers: no parsing, no
