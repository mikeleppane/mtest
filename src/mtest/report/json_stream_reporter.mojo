"""The live-writing machine-stream reporter: `JsonStreamReporter` (Layer 2).

Where `json_stream` is the PURE serializer (`Event` -> one NDJSON line), this is
its SINK: a `Reporter` that owns one resolved destination descriptor and writes
each serialized line to it live, as the session emits events. It is the machine
twin of `ConsoleReporter` — same seam (`handle`), different medium — and the one
place in the report layer that performs I/O.

Three properties matter and are all tested:

- **Header first.** Construction of an ACTIVE reporter writes the frozen
  `stream_header(version)` line before any event, so a consumer sees the version
  on line 1.
- **A write_all loop.** Every line is drained through a retry loop that advances
  on partial writes and retries `EINTR`; the reporter never assumes one `write`
  empties the buffer.
- **A status latch.** On ANY write failure the reporter latches — it records the
  failure and the `errno`, and EVERY later `handle` is a no-op that never writes
  and never raises. The concrete `status()` accessor (deliberately OUTSIDE the
  `Reporter` trait) lets an owner poll the latch and treat a dead stream as a
  fatal condition; `handle` itself stays total and non-raising per the trait.

An INERT reporter (`inert()`) is the no-`--json` shape: it owns no descriptor,
writes nothing, emits no header, and never latches. It exists so the session's
reporter composition can carry a stream slot unconditionally (a fixed tuple
...
def _raw_write[o: Origin](fd: Int, ptr: UnsafePointer[UInt8, o], n: Int) -> Int:
    """One `write(2)` of `n` bytes at `ptr` to `fd`; returns the syscall result.

    Returns the number of bytes written (possibly short), or a negative value on
    error (with `errno` set). The pointer is passed as an opaque byte pointer to
    match the stdlib's own `write` external declaration, so the symbol is not
    declared twice in one binary.
    """
...
def open_json_fd(path: String) raises -> Int:
    ...
    The caller owns the returned descriptor and must `close_json_fd` it. A
    failure to open (a missing parent directory that slipped past parse-time
    validation, a permission denial, descriptor exhaustion) raises — the caller
...
struct StreamStatus(Copyable, Movable):
    ...
    cause of the first failed write (0 when never failed, or when a zero-length
    write reported no progress). `context` names what was being written when the
    latch tripped, for a diagnostic.
    """
...
struct JsonStreamReporter(Reporter):
    ...
    and writes the line plus a newline through the write_all loop. On any write
    failure the reporter latches and goes silent. `Copyable, Movable` (all fields
    are trivial or owned `String`s) so it composes into the reporter tuple.
    """

    var _fd: Int
    """The destination descriptor; `-1` when inert. Borrowed, never closed here.
    """
    var _active: Bool
    """Whether this reporter writes at all; `False` is the inert no-`--json` shape.
    """
    var _failed: Bool
    ...
    def handle(mut self, e: Event):
        """Serialize `e` and write its NDJSON line. Total; never raises.

        A no-op when inert or already latched — the reporter never writes again
        after a failure and never raises out of the seam.
        """
    ...
    def _write_all(mut self, s: String, context: String) -> Bool:
        ...
        Returns `True` when every byte was written, `False` after latching a
        failure. Never assumes one `write` empties the buffer: it advances by the
        returned count on a short write and retries an `EINTR`. A non-`EINTR`
        error, or a zero-length write reporting no progress, latches the reporter.
        """
        var b = s.as_bytes()
        var total = len(b)
        var offset = 0
