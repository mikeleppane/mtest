...
def test_flow_pass_fail_excluded_warning_exit1() raises:
    ...
    # Full ordered stream: 1 start + 1 excluded + 1 warning + 2 file pairs + 1
    # finish = 8 events.
    ...
    # Run files, sorted: test_a_pass then test_b_fail, each a started/finished
    # pair.
    assert_true(rec.kind_at(3) == EventKind.FILE_STARTED)
    assert_equal(rec.path_at(3), "tests/test_a_pass.mojo")
    assert_true(rec.kind_at(4) == EventKind.FILE_FINISHED)
    assert_true(rec.outcome_at(4) == Outcome.PASS)
    assert_equal(rec.path_at(4), "tests/test_a_pass.mojo")

    assert_true(rec.kind_at(5) == EventKind.FILE_STARTED)
    ...
    assert_true(comp.reporters[1].kind_at(7) == EventKind.SESSION_FINISHED)
