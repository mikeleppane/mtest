...
entry beside it, which is seconds of I/O; a case that disables BEFORE reaching
the toolchain costs nothing. `test_env_base_enabled_for_default_config` is the
one test here that pays that price, and it is kept away from the other tests
that also pay it so no single suite carries two of them into one per-file
deadline. The early-refusal cases ride along because they are the same
question — does this config key? — answered the other way.
...
def test_env_base_enabled_for_default_config() raises:
    var root = temp_root()
    var ctx = env_base(base_config(), root)
