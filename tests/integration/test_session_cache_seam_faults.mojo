...
Two drivers reach that space and share this module: the dispatch faults the
native testing adapter injects, and the retry paths a crash or a killed compile
drive on their own. They share it because a classified module is its own
program and compiles the whole `mtest.session` import closure by itself, so a
runner with no compiler cache pays that closure once per module. Every config
here comes from `session_fixtures.base_config`, which names a wrapper rather
than the compiler itself, so nothing in either driver keys the toolchain's own
bytes and neither pays for the other's presence. The fault adapter is
...
# neither result; a compile killed mid-flight leaves a staging directory that
# must never be published or counted; and a precompile stamp replaced by a
# directory is refused rather than read. In each case the counters and the store
# have to agree that nothing was added.
