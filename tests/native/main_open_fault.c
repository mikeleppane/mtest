...
static void verify_main_repair_at_exit(void) {
    ...
    uint32_t restore_attempts = mtest_exec_test_fault_seen(MTEST_EXEC_OP_SIGACTION_RESTORE_INT);
    ...
    int reclose = final_reopen == 0 ? mtest_exec_runtime_close(&error) : -2;
    fprintf(stderr,
            "main-open-probe: restore-attempts-before-atexit=%u "
            "initial-reopen=%d repair=%d final-reopen=%d reclose=%d\n",
            restore_attempts, initial_reopen, repair, final_reopen, reclose);
    (void)fflush(stderr);
}

__attribute__((constructor)) static void configure_main_open_failure(void) {
    ...
    if (atexit(verify_main_repair_at_exit) != 0 ||
        mtest_exec_test_fault_configure(MTEST_EXEC_OP_SIGACTION_INSTALL_TERM, 1, EIO, 0) != 0 ||
        mtest_exec_test_fault_configure(MTEST_EXEC_OP_SIGACTION_RESTORE_INT, 1, EPERM, 0) != 0 ||
        mtest_exec_test_fault_configure_secondary(MTEST_EXEC_OP_SIGACTION_RESTORE_INT, 2, EIO, 0) !=
            0) {
