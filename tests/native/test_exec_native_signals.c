...
static int check_child_sigpipe_restored(const char *self_path) {
    ...
    struct mtest_exec_bytes probe_argv[2] = {bytes(self_path), bytes("__sigpipe_probe__")};
    ...
    for (int attempt = 0; attempt < 500 && !waitable; ++attempt) {
        ...
        if (mtest_exec_process_observe(process.handle, &observation, &error) != 0) {
    ...
    if (mtest_exec_process_channel_close(process.handle, MTEST_EXEC_CHANNEL_STDOUT, &error) != 0 ||
        mtest_exec_process_channel_close(process.handle, MTEST_EXEC_CHANNEL_STDERR, &error) != 0 ||
        mtest_exec_process_channel_close(process.handle, MTEST_EXEC_CHANNEL_SETUP, &error) != 0) {
...
static int matches_custom(int signal_number) {
    ...
    return current.sa_handler == custom_handler && (current.sa_flags & SA_RESTART) != 0 &&
...
int main(int argc, char **argv) {
    ...
    if (sigemptyset(&custom.sa_mask) != 0 || sigaddset(&custom.sa_mask, SIGUSR1) != 0 ||
        sigaction(SIGINT, &custom, &old_int) != 0 || sigaction(SIGTERM, &custom, &old_term) != 0 ||
        sigaction(SIGCHLD, &custom, &old_chld) != 0) {
        perror("install custom handlers");
        return 1;
    }

    if (mtest_exec_test_fault_configure(MTEST_EXEC_OP_SIGACTION_INSTALL_TERM, 1, EIO, 0) != 0 ||
        mtest_exec_runtime_open(&error) != -1 ||
        error.operation != MTEST_EXEC_OP_SIGACTION_INSTALL_TERM || error.error_number != EIO ||
        !matches_custom(SIGINT) || !matches_custom(SIGTERM) || !matches_custom(SIGCHLD)) {
        fprintf(stderr, "transactional install did not restore custom actions\n");
        return 1;
    }

    mtest_exec_test_fault_reset();
    if (mtest_exec_test_deliver_interrupt_after(MTEST_EXEC_OP_SIGACTION_INSTALL_INT, SIGINT) != 0 ||
        mtest_exec_runtime_open(&error) != 0 || mtest_exec_interrupt_requested() != 1 ||
    ...
    if (mtest_exec_interrupt_requested() != 0 || mtest_exec_runtime_close(&error) != 0 ||
        !matches_custom(SIGINT) || !matches_custom(SIGTERM) || !matches_custom(SIGCHLD)) {
        fprintf(stderr, "explicit close did not restore custom actions\n");
        return 1;
    }

    if (sigaction(SIGINT, &old_int, NULL) != 0 || sigaction(SIGTERM, &old_term, NULL) != 0 ||
    ...
    if (sigemptyset(&dfl_pipe.sa_mask) != 0 || sigaction(SIGPIPE, &dfl_pipe, &old_pipe) != 0) {
