...
static int exercise_platform_constants(void) {
    ...
    CHECK(mtest_exec_test_constant(MTEST_EXEC_TEST_CONSTANT_SIGCHLD) == SIGCHLD,
          "SIGCHLD constant is header-derived");
    CHECK(mtest_exec_test_constant(MTEST_EXEC_TEST_CONSTANT_EIO) == EIO,
          "EIO constant is header-derived");
    CHECK(mtest_exec_test_constant(MTEST_EXEC_TEST_CONSTANT_ETXTBSY) == ETXTBSY,
          "ETXTBSY constant is header-derived");
    CHECK(mtest_exec_test_constant(0) == -1, "zero constant id is rejected");
    CHECK(mtest_exec_test_constant(UINT32_MAX) == -1, "unknown constant id is rejected");
    return 0;
}

static int drain_channel(uint64_t handle, uint32_t channel, char *capture, size_t capacity,
                         size_t *length, int *eof) {
    uint8_t buffer[64];
    ...
    if (mtest_exec_process_read(handle, channel, buffer, sizeof(buffer), &result, &error) != 0) {
...
static int exercise_process(void) {
    struct mtest_exec_bytes argv[] = {bytes("/bin/sh"), bytes("-c"),
    ...
    struct mtest_exec_error error;
    CHECK(mtest_exec_process_open(&spec, &process, &error) == 0, "process open");
    ...
    CHECK(second.handle != process.handle, "concurrent children carry distinct handles");
    ...
    for (int attempt = 0; attempt < 500 && !waitable; ++attempt) {
        ...
        CHECK(mtest_exec_process_poll(process.handle, 10, &poll_result, &error) == 0,
              "process poll");
        CHECK(mtest_exec_process_setup_drain(process.handle, &setup, &error) == 0, "setup drain");
        CHECK(drain_channel(process.handle, MTEST_EXEC_CHANNEL_STDOUT, stdout_capture,
                            sizeof(stdout_capture), &stdout_length, &stdout_eof) == 0,
              "stdout drain");
        CHECK(drain_channel(process.handle, MTEST_EXEC_CHANNEL_STDERR, stderr_capture,
                            sizeof(stderr_capture), &stderr_length, &stderr_eof) == 0,
              "stderr drain");
        struct mtest_exec_observe_result observation;
        CHECK(mtest_exec_process_observe(process.handle, &observation, &error) == 0,
              "waitid observation");
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_GROUP_KILL, 1, EPERM, 0) == 0,
          "configure observed group-kill permission failure");
    CHECK(mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_KILL, &group, &error) == -1,
          "injected observed group-kill failure remains visible");
    CHECK(error.operation == MTEST_EXEC_OP_GROUP_KILL && error.error_number == EPERM,
    ...
    CHECK(mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_KILL, &group, &error) == 0,
          "group sweep");

    for (int attempt = 0;
         attempt < 100 && (!stdout_eof || !stderr_eof || setup.outcome == MTEST_EXEC_SETUP_WAITING);
         ++attempt) {
        struct mtest_exec_poll_result poll_result;
        CHECK(mtest_exec_process_poll(process.handle, 10, &poll_result, &error) == 0,
              "post-observe poll");
        CHECK(mtest_exec_process_setup_drain(process.handle, &setup, &error) == 0,
    ...
    CHECK(setup.outcome == MTEST_EXEC_SETUP_EXEC_SUCCEEDED, "setup protocol reports exec success");

    struct mtest_exec_reap_result reap;
    CHECK(mtest_exec_process_reap(process.handle, &reap, &error) == 0, "waitpid reap");
    CHECK(reap.kind == MTEST_EXEC_REAP_EXITED && reap.value == 7, "exact exit status 7");

    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_GROUP_KILL, 1, EIO, 0) == 0,
          "configure post-reap group-kill fault");
    CHECK(mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_KILL, &group, &error) == -1,
          "post-reap group action is rejected");
    CHECK(error.operation == MTEST_EXEC_OP_GROUP_KILL && error.error_number == EINVAL,
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_GROUP_TERM, 1, EIO, 0) == 0,
...
static int exercise_preobserve_zombie_only_term(void) {
    ...
    for (int attempt = 0; attempt < 500 && !waitable; ++attempt) {
        ...
        CHECK(waitid(P_PID, (id_t)process.leader_pid, &information, WEXITED | WNOHANG | WNOWAIT) ==
                  0,
    ...
    CHECK(mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_PROBE, &group, &error) == -1,
          "pre-observe zombie-only group probe remains fail-closed");
    CHECK(error.operation == MTEST_EXEC_OP_GROUP_PROBE && error.error_number == EPERM,
          "pre-observe zombie-only group probe error is exact");
#endif
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_GROUP_TERM, 1, EPERM, 0) == 0,
          "configure pre-observe group-TERM permission failure");
    CHECK(mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_TERM, &group, &error) == -1,
          "injected pre-observe group-TERM failure remains visible");
    CHECK(error.operation == MTEST_EXEC_OP_GROUP_TERM && error.error_number == EPERM,
    ...
    mtest_exec_test_fault_reset();
    CHECK(mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_TERM, &group, &error) == 0,
...
static int exercise_reaped_unswept_abort_child(void) {
    ...
    CHECK(mtest_exec_process_channel_close(process.handle, MTEST_EXEC_CHANNEL_STDOUT, &error) == 0,
          "close unswept stdout channel");
    CHECK(mtest_exec_process_channel_close(process.handle, MTEST_EXEC_CHANNEL_STDERR, &error) == 0,
          "close unswept stderr channel");
    CHECK(mtest_exec_process_channel_close(process.handle, MTEST_EXEC_CHANNEL_SETUP, &error) == 0,
          "close unswept setup channel");
    struct mtest_exec_reap_result reap;
    CHECK(mtest_exec_process_reap(process.handle, &reap, &error) == 0, "reap unswept leader");
    ...
    CHECK(error.operation == MTEST_EXEC_OP_NONE && error.error_number == EBUSY,
...
static int exercise_terminal_setpgid_cleanup_child(void) {
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_PARENT_SETPGID, 1, EIO, 0) == 0,
          "configure terminal parent setpgid failure");
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_WAITPID, 1, EPERM, 0) == 0,
    ...
    CHECK(error.operation == MTEST_EXEC_OP_PARENT_SETPGID && error.error_number == EIO,
          "terminal cleanup retains setpgid error as primary");
    CHECK(error.cleanup_operation == MTEST_EXEC_OP_WAITPID && error.cleanup_error_number == EPERM,
...
static int exercise_process_with_closed_standard_fds_child(void) {
    ...
    int saved_stdout = fcntl(STDOUT_FILENO, F_DUPFD_CLOEXEC, STDERR_FILENO + 1);
    if (saved_stdout < 0) {
    ...
    return setup_failed || restoration_failed || cleanup_failed ? 1 : exercise_result;
}
...
static int exercise_second_candidate_allocation_failure(void) {
    ...
    struct mtest_exec_process_ref process;
    struct mtest_exec_error error;
    int configure_result = mtest_exec_test_fault_configure(MTEST_EXEC_OP_PLAN_ALLOC, 7, ENOMEM, 0);
    int open_result = -1;
    if (configure_result == 0) {
        open_result = mtest_exec_process_open(&spec, &process, &error);
    }
    uint32_t allocation_attempts = mtest_exec_test_fault_seen(MTEST_EXEC_OP_PLAN_ALLOC);
    mtest_exec_test_fault_reset();
    ...
    int restore_result = saved_path == NULL ? unsetenv("PATH") : setenv("PATH", saved_path, 1);
    int restore_errno = errno;
    free(saved_path);

    CHECK(configure_result == 0, "configure second candidate allocation fault");
    CHECK(open_result == -1, "second candidate allocation fails open");
    CHECK(error.operation == MTEST_EXEC_OP_PLAN_ALLOC && error.error_number == ENOMEM,
          "candidate allocation error is exact");
    CHECK(allocation_attempts == 8, "interior NULL is followed by the third candidate allocation");
...
static int exercise_consumed_close_error(void) {
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_CLOSE_CHANNEL, 1, EIO, 0) == 0,
          "configure consumed close error");
    CHECK(mtest_exec_process_channel_close(process.handle, MTEST_EXEC_CHANNEL_STDOUT, &error) == -1,
          "close reports injected post-syscall error");
    CHECK(error.operation == MTEST_EXEC_OP_CLOSE_CHANNEL && error.error_number == EIO,
          "consumed close error is exact");
    int consumed_fd = (int)error.subject;
    int replacement = open("/dev/null", O_RDONLY);
    CHECK(replacement == consumed_fd, "closed descriptor number is immediately reusable");
    mtest_exec_test_fault_reset();
    CHECK(mtest_exec_process_abort(process.handle, 0, &error) == 0,
          "abort succeeds after consumed channel close");
    CHECK(fcntl(replacement, F_GETFD) >= 0, "abort never closes the replacement descriptor");
...
static int exercise_retryable_abort_group_kill(void) {
    ...
    CHECK(error.cleanup_operation == MTEST_EXEC_OP_GROUP_TERM && error.cleanup_error_number == EIO,
          "abort preserves the first group cleanup error");
    CHECK(mtest_exec_test_fault_seen(MTEST_EXEC_OP_GROUP_KILL) == 1,
          "abort attempts the failing group sweep exactly once");
    CHECK(kill(process.leader_pid, 0) == 0, "failed group sweep leaves the unreaped leader live");
    siginfo_t information;
    memset(&information, 0, sizeof(information));
    CHECK(waitid(P_PID, (id_t)process.leader_pid, &information, WEXITED | WNOHANG | WNOWAIT) == 0,
          "inspect leader after failed group sweep");
    CHECK(information.si_pid == 0, "failed group sweep never kills or makes the leader waitable");
    mtest_exec_test_fault_reset();
    CHECK(mtest_exec_process_abort(process.handle, 0, &error) == 0,
...
static int exercise_transient_group_signal_eperm(void) {
    ...
    CHECK(mtest_exec_test_group_signal_eperm_configure(MTEST_EXEC_OP_GROUP_KILL, 2) == 0,
    ...
    CHECK(mtest_exec_test_group_signal_eperm_seen(MTEST_EXEC_OP_GROUP_KILL) == 0,
          "injected group-kill EPERM bypasses transient syscall retry");
    CHECK(mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_KILL, &group, &error) == 0,
          "transient group-kill EPERM is retried");
    CHECK(group.state == MTEST_EXEC_GROUP_PRESENT, "retried group kill reports present");
    CHECK(mtest_exec_test_group_signal_eperm_seen(MTEST_EXEC_OP_GROUP_KILL) == 3,
    ...
    CHECK(mtest_exec_test_group_signal_eperm_configure(MTEST_EXEC_OP_GROUP_TERM, 100) == 0,
    ...
    CHECK(mtest_exec_test_group_signal_eperm_seen(MTEST_EXEC_OP_GROUP_TERM) == 21,
...
static int exercise_slot_capacity(void) {
    ...
    CHECK(error.error_number == EBUSY, "capacity exhaustion reports EBUSY");
    CHECK(error.detail == 0, "slot-exhaustion EBUSY carries detail 0, not the pin's detail 1");
...
static int exercise_stale_token(void) {
    ...
    CHECK(mtest_exec_process_open(&spec, &reused, &error) == 0, "the freed slot is reusable");
    CHECK(reused.handle != stale, "the reused slot publishes a fresh token, never the stale one");
...
static int exercise_poll_set_readiness(void) {
    ...
    struct mtest_exec_bytes argv_a[] = {bytes("/bin/sh"), bytes("-c"),
                                        bytes("printf ready; exit 0")};
    struct mtest_exec_bytes argv_b[] = {bytes("/bin/sh"), bytes("-c"),
                                        bytes("printf also >&2; exit 0")};
...
static int exercise_poll_set_hostile_structural(void) {
    ...
    for (size_t index = 0; index < sizeof(cases) / sizeof(cases[0]); ++index) {
        ...
        CHECK(mtest_exec_poll_set(cases[index].handle_ptr, cases[index].count,
                                  cases[index].timeout_ms, cases[index].result_ptr, &error) == -1,
              cases[index].label);
        CHECK(error.error_number == EINVAL, "structural violation reports EINVAL");
...
static int exercise_poll_set_hostile_semantic(void) {
    ...
    CHECK(results[0].readiness == 0 && results[0].reserved == 0 && results[1].readiness == 0 &&
              results[1].reserved == 0,
...
static int exercise_handle_scoped_fault(void) {
    ...
    struct mtest_exec_group_result group;
    CHECK(mtest_exec_test_fault_configure_handle(b.handle, MTEST_EXEC_OP_GROUP_TERM, 1, EIO) == 0,
          "configure a handle-scoped group-TERM fault on b");
    CHECK(mtest_exec_process_group(a.handle, MTEST_EXEC_GROUP_TERM, &group, &error) == 0,
    ...
    CHECK(mtest_exec_process_group(b.handle, MTEST_EXEC_GROUP_TERM, &group, &error) == -1,
          "b's group TERM fires the scoped fault");
    CHECK(error.operation == MTEST_EXEC_OP_GROUP_TERM && error.error_number == EIO,
...
static int run_capture(const struct mtest_exec_process_spec *spec, char *out, size_t out_cap,
                       size_t *out_len, uint32_t *setup_outcome, int *reap_value) {
    struct mtest_exec_process_ref process;
    struct mtest_exec_error error;
    ...
    *out_len = 0;
    ...
    for (int attempt = 0; attempt < 3000 && !waitable; ++attempt) {
        ...
        if (mtest_exec_process_poll(process.handle, 10, &pr, &error) != 0 ||
            ...
            drain_channel(process.handle, MTEST_EXEC_CHANNEL_STDOUT, out, out_cap, out_len,
                          &out_eof) != 0 ||
            drain_channel(process.handle, MTEST_EXEC_CHANNEL_STDERR, errbuf, sizeof(errbuf),
                          &err_len, &err_eof) != 0 ||
    ...
    struct mtest_exec_group_result group;
    if (mtest_exec_process_group(process.handle, MTEST_EXEC_GROUP_KILL, &group, &error) != 0) {
        return -1;
    }
    for (int attempt = 0;
         attempt < 300 && (!out_eof || !err_eof || setup.outcome == MTEST_EXEC_SETUP_WAITING);
         ++attempt) {
        struct mtest_exec_poll_result pr;
        if (mtest_exec_process_poll(process.handle, 10, &pr, &error) != 0 ||
            mtest_exec_process_setup_drain(process.handle, &setup, &error) != 0 ||
...
static int exercise_env_validation(void) {
    ...
    for (size_t index = 0; index < 3; ++index) {
        ...
        CHECK(mtest_exec_process_open(&spec, &process, &error) == -1, labels[index]);
        CHECK(error.operation == MTEST_EXEC_OP_ENV_SNAPSHOT && error.error_number == EINVAL,
              "malformed env extra reports the env snapshot EINVAL");
    }

    struct mtest_exec_bytes duplicate[2] = {bytes("MTEST_ENVX=one"), bytes("MTEST_ENVX=two")};
    memset(&spec, 0, sizeof(spec));
    spec.argv = argv;
    spec.argc = 1;
...
static int exercise_env_merge(void) {
    ...
    spec.env_extra_count = 1;
    CHECK(run_capture(&spec, out, sizeof(out), &out_len, &setup_outcome, &reap_value) == 0,
          "override run completes");
    size_t envx_lines = 0;
    for (const char *cursor = out; (cursor = strstr(cursor, "MTEST_ENVX=")) != NULL; cursor += 1) {
        if (cursor == out || cursor[-1] == '\n') {
            envx_lines += 1;
        }
    }
    CHECK(envx_lines == 1, "the inherited entry is dropped, not appended beside the override");
    ...
    struct mtest_exec_bytes argv_novel[] = {bytes("/bin/sh"), bytes("-c"),
                                            bytes("printf '%s' \"$MTEST_ENVY\"")};
    ...
    struct mtest_exec_bytes path_override = bytes("PATH=/mtest-nonexistent-dir");
    memset(&spec, 0, sizeof(spec));
    spec.argv = argv_path;
    spec.argc = 1;
    ...
    struct mtest_exec_bytes argv_base[] = {bytes("/bin/sh"), bytes("-c"),
                                           bytes("printf '%s' \"$MTEST_ENVX\"")};
    memset(&spec, 0, sizeof(spec));
    spec.argv = argv_base;
    spec.argc = 3;
    CHECK(run_capture(&spec, out, sizeof(out), &out_len, &setup_outcome, &reap_value) == 0,
          "zero-override run completes");
    CHECK(strcmp(out, "inherited") == 0, "a zero override count reproduces the v1 environment");
...
static int exercise_fd_limit(void) {
    ...
    if (saved.rlim_cur == RLIM_INFINITY) {
        ...
        CHECK(reported == (uint64_t)saved.rlim_cur, "the reported soft limit equals getrlimit");
    }
    ...
    CHECK(reported == (uint64_t)reduced.rlim_cur, "fd limit tracks the lowered soft limit");
...
static int exercise_fd_limit_peak_child(void) {
    ...
    int ok =
        boundary_open == 0 && past_open == -1 && (past_error == EMFILE || past_error == ENFILE);
...
static int exercise_emfile_pipe_cleanup(void) {
    ...
    mtest_exec_test_fault_reset();
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_PIPE_STDOUT, 1, EMFILE, 0) == 0,
          "configure EMFILE at the stdout pipe seam");
    CHECK(mtest_exec_process_open(&spec, &process, &error) == -1,
          "EMFILE at pipe creation rejects open");
    CHECK(error.operation == MTEST_EXEC_OP_PIPE_STDOUT && error.error_number == EMFILE,
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_PIPE_STDERR, 1, EMFILE, 0) == 0,
          "configure EMFILE at the stderr pipe seam");
    CHECK(mtest_exec_process_open(&spec, &process, &error) == -1,
          "EMFILE at the second pipe rejects open");
    CHECK(error.operation == MTEST_EXEC_OP_PIPE_STDERR && error.error_number == EMFILE,
...
static int exercise_interrupt_counting(void) {
    ...
    CHECK(mtest_exec_interrupt_count() == 1, "one observed activation counts 1");

    /* Saturation: further activations saturate at 2 and never overflow. */
    CHECK(kill(getpid(), SIGTERM) == 0, "deliver SIGTERM");
    CHECK(mtest_exec_interrupt_count() == 2, "a second observed activation escalates to 2");
    CHECK(kill(getpid(), SIGINT) == 0, "deliver a third signal");
    CHECK(mtest_exec_interrupt_count() == 2, "the count saturates at 2");
    mtest_exec_test_reset_interrupt();
...
int main(void) {
    ...
    struct mtest_exec_error error;
    CHECK(mtest_exec_native_abi_version() == MTEST_EXEC_NATIVE_ABI_VERSION, "ABI version");
    CHECK(mtest_exec_interrupt_requested() == 0, "interrupt is initially clear");
    CHECK(mtest_exec_runtime_open(&error) == 0, "runtime open");
    CHECK(mtest_exec_runtime_open(&error) == -1 && error.error_number == EBUSY,
    ...
    int64_t now = 0;
    CHECK(mtest_exec_monotonic_ms(&now, &error) == 0 && now > 0, "monotonic clock");
    ...
    CHECK(exercise_poll_set_hostile_structural() == 0, "poll_set structural hostile callers");
    CHECK(exercise_poll_set_hostile_semantic() == 0, "poll_set semantic hostile callers");
    ...
    CHECK(exercise_emfile_pipe_cleanup() == 0, "EMFILE pipe-creation cleanup under Valgrind");
    ...
    CHECK(exercise_consumed_close_error() == 0, "consumed close error ownership");
    CHECK(exercise_retryable_abort_group_kill() == 0, "retryable abort group kill");
    CHECK(exercise_transient_group_signal_eperm() == 0, "transient group-signal EPERM retry");
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_CHILD_SETUP_WRITE, 1, EIO, 9) == -1,
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_FD_CLOEXEC, 2, EIO, 0) == 0,
          "configure occurrence-specific fd fault");
    CHECK(mtest_exec_process_open(&fault_spec, &fault_process, &error) == -1,
          "second adapter-local fcntl occurrence fails");
    CHECK(error.operation == MTEST_EXEC_OP_FD_CLOEXEC && error.error_number == EIO &&
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_WAITPID, 1, EINTR, 0) == 0,
    ...
    mtest_exec_test_fault_reset();
    CHECK(mtest_exec_process_open(&fault_spec, &fault_process, &error) == 0,
          "runtime is reusable after proven setpgid cleanup");
    CHECK(mtest_exec_process_abort(fault_process.handle, 0, &error) == 0,
          "ordinary lifecycle follows setpgid cleanup");

    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_PARENT_SETPGID, 1, EIO, 0) == 0,
          "configure parent setpgid failure before ECHILD");
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_WAITPID, 1, ECHILD, 0) == 0,
    ...
    CHECK(error.cleanup_operation == MTEST_EXEC_OP_NONE && error.cleanup_error_number == 0,
    ...
    CHECK(echid_waited == failed_leader || (echid_waited < 0 && errno == ECHILD),
    ...
    CHECK(mtest_exec_process_abort(fault_process.handle, 0, &error) == -1,
          "abort retains TERM error after successful cleanup");
    CHECK(error.operation == MTEST_EXEC_OP_NONE && error.error_number == 0 &&
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_CLOCK_MONOTONIC, 1, EPERM, 0) == 0,
          "configure abort clock failure after TERM failure");
    CHECK(mtest_exec_process_abort(fault_process.handle, 0, &error) == -1,
          "abort retains TERM error beside primary clock error");
    CHECK(error.operation == MTEST_EXEC_OP_CLOCK_MONOTONIC && error.error_number == EPERM &&
              error.cleanup_operation == MTEST_EXEC_OP_GROUP_TERM &&
              error.cleanup_error_number == EIO,
    ...
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_CLOCK_MONOTONIC, 1, EIO, 0) == 0,
          "configure abort clock failure");
    CHECK(mtest_exec_process_abort(fault_process.handle, 0, &error) == -1,
          "abort never launders primary machinery error");
    CHECK(error.operation == MTEST_EXEC_OP_CLOCK_MONOTONIC && error.error_number == EIO,
    ...
    mtest_exec_test_fault_configure(MTEST_EXEC_OP_SIGACTION_INSTALL_TERM, 1, EIO, 0);
    CHECK(mtest_exec_runtime_open(&error) == -1, "injected second install failure");
    CHECK(error.operation == MTEST_EXEC_OP_SIGACTION_INSTALL_TERM && error.error_number == EIO &&
              error.cleanup_operation == MTEST_EXEC_OP_NONE,
          "install failure preserves initiating error and rolls back");
    mtest_exec_test_fault_reset();

    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_SIGACTION_INSTALL_TERM, 1, EIO, 0) == 0,
          "configure initiating install fault");
    CHECK(mtest_exec_test_fault_configure(MTEST_EXEC_OP_SIGACTION_RESTORE_INT, 1, EPERM, 0) == 0,
          "configure rollback fault");
    CHECK(mtest_exec_runtime_open(&error) == -1, "injected install plus rollback failure");
    CHECK(error.operation == MTEST_EXEC_OP_SIGACTION_INSTALL_TERM && error.error_number == EIO &&
    ...
    CHECK(mtest_exec_runtime_open(&error) == 0, "runtime opens after transactional rollback");

    mtest_exec_test_fault_configure(MTEST_EXEC_OP_SIGACTION_RESTORE_TERM, 1, EIO, 0);
    CHECK(mtest_exec_runtime_close(&error) == -1, "injected restoration failure is fallible");
    CHECK(error.operation == MTEST_EXEC_OP_SIGACTION_RESTORE_TERM && error.error_number == EIO,
    ...
    CHECK(mtest_exec_process_open(&blocked_spec, &blocked_process, &error) == -1 &&
              error.error_number == EINVAL,
          "partial teardown blocks new child ownership");
    mtest_exec_test_fault_reset();
    CHECK(mtest_exec_runtime_close(&error) == 0, "explicit close retry finishes restoration");

    CHECK(mtest_exec_runtime_open(&error) == 0, "sequential runtime reinstall");
    CHECK(mtest_exec_runtime_close(&error) == 0, "sequential runtime teardown");
