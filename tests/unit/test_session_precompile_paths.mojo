...
cannot damage a good package left by an earlier run. Five properties make that
hold, and all five are pinned here: the temp is NEVER equal to OUT (a promotion
...
one root — must not unlink each other's temp dir mid-compile), the temp stays
UNDER OUT's directory (a rename is atomic only within one filesystem — across one
it silently becomes a copy), and it is NOT IN that directory: `-I <out-dir>` scans
OUT's directory, and the toolchain forces a package OUT path to keep its
`.mojopkg`/`.mojoc` extension (`mojo precompile -o x.tmp` is a hard error), so the
ONLY place a still-unpromoted package can hide from the include scan is a `.tmp`
subdirectory of its own. Reached through the same private-helper seam
`test_session_mangle.mojo` uses.
"""
...
def test_temp_path_is_derived_from_out_with_attempt_index() raises:
    assert_equal(
        _precompile_temp_path("build/mathlib.mojopkg", "e2e/pkg/mathlib", 1),
        (
            "build/.mtest-precompile-e2e_spkg_smathlib.attempt-1.tmp/"
            "mathlib.mojopkg"
        ),
    )
    assert_equal(
        _precompile_temp_path("build/mathlib.mojopkg", "e2e/pkg/mathlib", 2),
        (
            "build/.mtest-precompile-e2e_spkg_smathlib.attempt-2.tmp/"
            "mathlib.mojopkg"
        ),
    )
...
def test_temp_path_never_equals_out() raises:
    ...
    for k in range(1, 5):
        assert_false(
            _precompile_temp_path(out, "pkg/mathlib", k) == out,
...
def test_each_attempt_gets_a_distinct_temp_path() raises:
    ...
    for k in range(1, 5):
        var t = _precompile_temp_path(out, "pkg/mathlib", k)
        for s in seen:
...
def test_each_step_gets_a_distinct_temp_path() raises:
    ...
    var a = _precompile_temp_path("build/lib.mojopkg", "pkg/one", 1)
    var b = _precompile_temp_path("build/lib.mojopkg", "pkg/two", 1)
...
def test_temp_path_stays_under_the_out_directory() raises:
    ...
    assert_equal(
        dirname(dirname(_precompile_temp_path(out, "pkg/mathlib", 1))),
        "out/pkgs",
    )
    # A bare OUT name resolves against the invocation root, so its temp does too.
    assert_equal(
        dirname(dirname(_precompile_temp_path("mathlib.mojopkg", "m", 1))), ""
    )
...
def test_temp_path_is_not_in_the_out_directory_itself() raises:
    ...
    var out = String("build/mathlib.mojopkg")
    var t = _precompile_temp_path(out, "pkg/mathlib", 1)
