...
capture). The junit-10 schema + arithmetic oracle (`scripts/formats/junit.py`) is
run over the REAL rendered output of these same cells by the
`scripts/formats/junit_render.py` CI gate; here the shapes and counts are pinned
